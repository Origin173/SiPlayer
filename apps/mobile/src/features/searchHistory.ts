...
let searchHistoryWriteQueue: Promise<void> = Promise.resolve();
...
export async function recordSearchKeyword(keyword: string): Promise<string[]> {
  ...
  const write = searchHistoryWriteQueue.then(async () => {
  ...
  searchHistoryWriteQueue = write.then(() => undefined, () => undefined);
  return write;
}
