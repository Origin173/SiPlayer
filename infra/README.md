...
- Use `-IncludeQr` to require the unauthenticated QR-start contract, and `-PlaylistId <id>` to verify playlist detail. Use `-TrackId <id>` to avoid selecting a track from search, or `-SkipNegativeChecks` only when the environment forbids safe validation probes.
