...
for (const [packageName, packagePath] of releasePackages) {
  const packageJson = JSON.parse(readFileSync(packagePath, 'utf8'));
  if (packageJson.version !== match[1]) {
    throw new Error(`Tag ${tag} does not match ${packageName} package version ${packageJson.version}. Update ${packagePath} before tagging.`);
  }
}
